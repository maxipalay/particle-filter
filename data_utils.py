...
class DataLoader:
    ...
    def __init__(self):
        ...
        self.barcodes_landmarks = self.barcodes[np.isin(self.barcodes[:,self.BARC_S],self.landmarks[:,self.LMT_S])][:,self.BARC_B]
    ...
    def get_measurements(self, timestamp1, timestamp2):
        ...
        """
        indexes = np.where((timestamp1 < self.measurements[:, self.MEAS_T]) & \
                           (self.measurements[:, self.MEAS_T] <= timestamp2) & \
                            (np.isin(self.measurements[:, self.BARC_B], self.barcodes_landmarks)))  # indexes of measurements between the given timesteps and that are landmarks
        x = self.measurements[indexes, :][0].copy()
        # replace the barcode ID with the landmark ID
        for i in range(x.shape[0]):
            x[i,self.MEAS_S] = self.barcodes[np.where(self.barcodes[:,self.BARC_B]==x[i,self.MEAS_S])[0][0]][self.BARC_S]
        return x
    ...
    def get_groundtruth(self, prev_timestamp, this_timestamp):
        ...
        return self.ground_truth[np.where((prev_timestamp < self.ground_truth[:,self.GT_T]) & (self.ground_truth[:,self.GT_T] <= this_timestamp)),:][0]
